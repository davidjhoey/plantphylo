...
usage() {
  cat <<'EOF'
Usage:
  phylominer1.1.sh [options] query.fasta /path/to/databases
...
Options:
  ...
  --motif-hmm HMM_ID       Filter extracted proteins by HMM profile (repeatable)
  --pfam-db PATH           Pressed HMM database for --motif-hmm
...
safe_rm() {
    ...
    if [[ "$file" == "$DB_DIR"/*.fa ]]; then
        echo "[WARN] Refusing to delete database:"
        echo "       $file"
        return
    fi
...
WORKDIR="$DB_DIR/phylominer_work"
mkdir -p "$WORKDIR"
...
for db in "${DB_FILES[@]}"; do
  ...
  case "$base" in
    hit_summary.csv|*_phmmer_output.txt|*_phmmer_tbl.txt|*_hit_ids.txt|*_homologs.prot.fasta|*_homologs.cds.fasta)
      continue
      ;;
  esac
  ...
  PHMMER_TXT="$DB_DIR/${name}_phmmer_output.txt"
  PHMMER_TBL="$DB_DIR/${name}_phmmer_tbl.txt"
  ID_LIST="$DB_DIR/${name}_hit_ids.txt"
  PROT_OUT="$DB_DIR/${name}_homologs.prot.fasta"
  CDS_OUT="$DB_DIR/${name}_homologs.cds.fasta"
  ...
  if ! phmmer --cpu "$THREADS" --noali --tblout "$PHMMER_TBL" "$QUERY_PROT" "$PROT_DB" > "$PHMMER_TXT" 2>&1; then
    ...
    echo "--------------------------------"
    printf '%s,%s,0,0,0,0,0,0,0,no,no\n' "$name" "$TOTAL_SEQS" >> "$SUMMARY_CSV"
    continue
  fi
  ...
  if [[ ${#MOTIF_HMMS[@]} -gt 0 ]]; then
    ...
    echo "[DEBUG] Before HMM: $BEFORE_HMM After HMM: $AFTER_HMM"
